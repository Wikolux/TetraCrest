"""DeliveryPlanner - a deterministic, rule-based planner implementing
SpecialistPlanner, mirroring DiscoveryPlanner/DecisionPlanner exactly.
Every task is tagged SpecialistTaskType.SUMMARIZATION - the category ARR
§3 names as this specialist's own fit ("SUMMARIZATION (SpecialistTaskType)")
- with the operation's real purpose carried in the task's title. The plan
is an observability artifact, not a step-by-step execution driver.
"""
...
TASK_GATHER_PRECEDENT = "Gather delivery precedent and evidence"
TASK_PROCESS_REQUEST = "Process delivery request"


class DeliveryPlanner(SpecialistPlanner):
