"""build_decision_context - mirrors test_discovery_context.py's own
...
def test_composes_agent_context_and_request():
    ...
    request = SpecialistRequest(objective="should we build this?")

    context = build_decision_context(agent_context, request)
...
def test_shared_execution_context_delegates_to_agent_context():
    ...
    context = build_decision_context(agent_context, SpecialistRequest(objective="x"))
