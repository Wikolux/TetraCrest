"""build_discovery_context - mirrors test_research_context.py's own
...
def test_composes_agent_context_and_request():
    ...
    request = SpecialistRequest(objective="validate this problem")

    context = build_discovery_context(agent_context, request)
...
def test_shared_execution_context_delegates_to_agent_context():
    ...
    context = build_discovery_context(agent_context, SpecialistRequest(objective="x"))
