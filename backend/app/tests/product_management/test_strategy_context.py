"""build_strategy_context - mirrors test_discovery_context.py's/
test_decision_context.py's/test_delivery_context.py's own coverage: a
thin composition, never a parallel context type.
"""
...
def test_composes_agent_context_and_request():
    ...
    request = SpecialistRequest(objective="sequence the roadmap")

    context = build_strategy_context(agent_context, request)
...
def test_shared_execution_context_delegates_to_agent_context():
    ...
    context = build_strategy_context(agent_context, SpecialistRequest(objective="x"))
