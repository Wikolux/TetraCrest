"""StrategyPortfolioSpecialist (CP-02, Milestone 6) - comprehensive
coverage: framework-appropriate operation behavior, memory integration
(writes, evidence-gating, append-only), the v1 single-product portfolio
scope limitation, Executive compatibility, PromptBuilder/Runtime
integration, event ordering, state transitions, failure handling,
architecture compliance, and Discovery/Decision/Delivery integration
through shared memory only.

No mocks - hand-written fakes only, mirroring test_discovery_agent.py's/
test_decision_agent.py's/test_delivery_agent.py's own conventions exactly.
"""

import ast
import importlib
import inspect
from datetime import UTC, datetime

import pytest

from app.services.ai.agents.base_agent import BaseAgent
from app.services.ai.agents.context import AgentContext
from app.services.ai.agents.enums import AgentCapability
from app.services.ai.agents.executive.dispatcher import Dispatcher
from app.services.ai.agents.executive.decision import Decision
from app.services.ai.agents.executive.task import Task
from app.services.ai.agents.factory import AgentFactory
from app.services.ai.agents.memory import AgentMemory
from app.services.ai.agents.registry import AgentRegistry
from app.services.ai.agents.specialists.factory import SpecialistFactory
from app.services.ai.agents.specialists.product_management.memory_service import ProfessionalMemoryService
from app.services.ai.agents.specialists.product_management.shared.types import MEMORY_TYPE_METRIC, MEMORY_TYPE_ROADMAP
from app.services.ai.agents.specialists.product_management.strategy_portfolio.events import (
    StrategyEventPublisher,
    StrategyEventType,
)
from app.services.ai.agents.specialists.product_management.strategy_portfolio.request import (
    StrategyOperation,
    StrategyRequest,
)
from app.services.ai.agents.specialists.product_management.strategy_portfolio.state import StrategyState
from app.services.ai.agents.specialists.product_management.strategy_portfolio.strategy_portfolio_agent import (
    StrategyPortfolioSpecialist,
)
from app.services.ai.agents.specialists.registry import SpecialistRegistry
from app.services.ai.agents.specialists.runtime_adapter import RuntimeAdapter
from app.services.ai.agents.specialists.shared.context import SpecialistContext
from app.services.ai.agents.specialists.shared.task import SpecialistTaskType
from app.services.ai.providers.enums import ProviderName
from app.services.ai.runtime.types import RuntimeResponse
from app.services.ai.shared.execution_metadata import ExecutionMetadata
from app.services.ai.shared.response import AIResponseMetadata, ProviderResponse
from app.services.ai.conversation.types import ConversationResponse
from app.services.context.types import ContextItem, ContextPackage, ContextSection


class _FakeRuntime:
    def __init__(self, response=None, raises=None):
        self.response = response if response is not None else RuntimeResponse(success=True)
        self.raises = raises
        self.requests = []

    def execute(self, request):
        self.requests.append(request)
        if self.raises is not None:
            raise self.raises
        return self.response


class _FakeMemory(AgentMemory):
    def __init__(self, package=None, per_query_packages=None):
        self.package = package or ContextPackage(sections=[], estimated_tokens=0, item_count=0, truncated=False)
        self.per_query_packages = per_query_packages or {}
        self.remembered = []
        self.retrieve_calls = []
        self.search_calls = []
        self.raises_on_remember = None

    def remember(self, item, **kwargs):
        if self.raises_on_remember is not None:
            raise self.raises_on_remember
        self.remembered.append((item, kwargs))

    def retrieve(self, query, **kwargs):
        self.retrieve_calls.append((query, kwargs))
        return self.per_query_packages.get(query, self.package)

    def search(self, query, **kwargs):
        self.search_calls.append((query, kwargs))
        return self.package

    def forget(self, item_id):
        raise NotImplementedError


class _FakeMemoryServiceRows:
    def __init__(self, rows=()):
        self.rows = list(rows)
        self.calls = []

    def list_memories(self, organization_id, skip=0, limit=20):
        self.calls.append((organization_id, skip, limit))
        matching = [row for row in self.rows if row.organization_id == organization_id]
        return matching[skip : skip + limit]


class _Row:
    def __init__(self, id, organization_id, content, memory_type, title="", created_at=None):
        self.id = id
        self.organization_id = organization_id
        self.content = content
        self.memory_type = memory_type
        self.title = title
        self.created_at = created_at or datetime.now(UTC)


def _memory_item(content="prior finding", resource_id=1):
    return ContextItem(
        resource_type="memory", resource_id=resource_id, content=content, score=1.0, created_at=datetime.now(UTC)
    )


def _discovery_item(resource_id=1):
    return _memory_item(content="Discovery finding: users churn. Source: tickets. Status: unknown.", resource_id=resource_id)


def _decision_item(resource_id=2):
    return _memory_item(
        content="Decision: Onboarding revamp. Framework applied: rice. Rationale: proceed.", resource_id=resource_id
    )


def _package_with_items(items):
    return ContextPackage(
        sections=[ContextSection(resource_type="memory", items=items)],
        estimated_tokens=10,
        item_count=len(items),
        truncated=False,
    )


def _full_evidence_package():
    return _package_with_items([_discovery_item(1), _decision_item(2)])


def _agent(**overrides):
    memory = overrides.pop("memory_adapter", None) or _FakeMemory()
    defaults = dict(
        runtime_adapter=RuntimeAdapter(runtime=_FakeRuntime()),
        memory_adapter=memory,
        memory_service=ProfessionalMemoryService(memory, _FakeMemoryServiceRows()),
        default_provider=ProviderName.OPENAI,
    )
    defaults.update(overrides)
    return StrategyPortfolioSpecialist(**defaults)


def _agent_context(**overrides):
    defaults = dict(organization_id=1, user_id=2)
    defaults.update(overrides)
    return AgentContext(**defaults)


def _specialist_context(agent_context=None):
    return SpecialistContext(agent_context=agent_context or _agent_context())


# --- construction / registration -----------------------------------------------------------


def test_is_a_base_agent():
    assert isinstance(_agent(), BaseAgent)


def test_is_registered_in_the_agent_registry():
    assert AgentRegistry.is_registered("strategy_portfolio")
    assert AgentRegistry.get("strategy_portfolio") is StrategyPortfolioSpecialist


def test_is_registered_in_the_specialist_registry():
    assert SpecialistRegistry.exists("strategy_portfolio")
    registration = SpecialistRegistry.get_registration("strategy_portfolio")
    assert registration.specialization == "strategy_portfolio"
    assert registration.supported_tasks == frozenset({SpecialistTaskType.ANALYSIS})


def test_constructible_via_agent_factory():
    agent = AgentFactory.create(
        "strategy_portfolio", runtime_adapter=RuntimeAdapter(runtime=_FakeRuntime()), memory_adapter=_FakeMemory()
    )
    assert isinstance(agent, StrategyPortfolioSpecialist)


def test_constructible_via_specialist_factory():
    agent = SpecialistFactory.create(
        "strategy_portfolio", runtime_adapter=RuntimeAdapter(runtime=_FakeRuntime()), memory_adapter=_FakeMemory()
    )
    assert isinstance(agent, StrategyPortfolioSpecialist)


def test_specialization_is_strategy_portfolio():
    assert _agent().specialization() == "strategy_portfolio"


def test_self_check_and_health_check_and_health_are_true_by_default():
    agent = _agent()
    assert agent.self_check() is True
    assert agent.health_check() is True
    assert agent.health() is True


def test_evaluate_uses_the_strategy_policy_minimum_confidence():
    from app.services.ai.agents.specialists.product_management.strategy_portfolio.policies import StrategyPolicy
    from app.services.ai.agents.specialists.shared.response import SpecialistResponse

    agent = _agent(strategy_policy=StrategyPolicy(minimum_confidence=0.5))
    assert agent.evaluate(SpecialistResponse(success=True, confidence=0.6)) is True
    assert agent.evaluate(SpecialistResponse(success=True, confidence=0.4)) is False


# --- capability declaration / Executive collision-avoidance -------------------------------


def test_declares_only_reasoning_and_planning():
    assert _agent().capabilities().declared == frozenset({AgentCapability.REASONING, AgentCapability.PLANNING})


def test_never_declares_memory_capability():
    assert AgentCapability.MEMORY not in _agent().capabilities().declared


def test_never_declares_research_capability():
    assert AgentCapability.RESEARCH not in _agent().capabilities().declared


# --- Executive / Dispatcher compatibility --------------------------------------------------


def test_dispatcher_routes_a_reasoning_tagged_task_to_strategy_portfolio_specialist():
    agent = _agent()
    task = Task(title="sequence roadmap", execution_id="exec-1", metadata={"required_capability": AgentCapability.REASONING})

    routed = Dispatcher().dispatch(Decision(), task, {"strategy_portfolio": agent})

    assert routed is agent


def test_dispatcher_never_routes_a_memory_tagged_task():
    agent = _agent()
    task = Task(title="retrieve_memory", execution_id="exec-1", metadata={"required_capability": AgentCapability.MEMORY})

    routed = Dispatcher().dispatch(Decision(), task, {"strategy_portfolio": agent})

    assert routed is None


# --- execute() / BaseAgent contract ---------------------------------------------------------


def test_execute_requires_organization_id():
    with pytest.raises(ValueError):
        _agent().execute(AgentContext(organization_id=None))


def test_execute_extracts_operation_from_agent_metadata():
    agent = _agent()
    context = _agent_context(agent_metadata=ExecutionMetadata(extra={"operation": "recall", "text": "roadmap"}))

    response = agent.execute(context)

    assert isinstance(response, RuntimeResponse)
    assert response.success is True


# --- memory integration: writes, evidence-gating, append-only -------------------------------


def test_sequence_roadmap_writes_roadmap_items_for_each_candidate():
    memory = _FakeMemory()
    agent = _agent(memory_adapter=memory)
    request = StrategyRequest(operation=StrategyOperation.SEQUENCE_ROADMAP, items=("Ship A", "Ship B"), horizon="now")

    response = agent.process(request, _specialist_context())

    assert response.success is True
    assert len(memory.remembered) == 2
    for content, kwargs in memory.remembered:
        assert kwargs["memory_type"] == MEMORY_TYPE_ROADMAP
        assert "now" in content


def test_structure_north_star_writes_a_north_star_metric():
    memory = _FakeMemory()
    agent = _agent(memory_adapter=memory)
    request = StrategyRequest(operation=StrategyOperation.STRUCTURE_NORTH_STAR, title="Weekly Active Teams")

    response = agent.process(request, _specialist_context())

    assert response.success is True
    assert len(memory.remembered) == 1
    content, kwargs = memory.remembered[0]
    assert kwargs["memory_type"] == MEMORY_TYPE_METRIC
    assert "North Star" in content


def test_structure_okrs_writes_one_metric_per_key_result():
    memory = _FakeMemory()
    agent = _agent(memory_adapter=memory)
    request = StrategyRequest(
        operation=StrategyOperation.STRUCTURE_OKRS, text="Improve retention", key_results=("Reduce churn by 10%", "Grow WAU by 5%")
    )

    agent.process(request, _specialist_context())

    assert len(memory.remembered) == 2
    for _, kwargs in memory.remembered:
        assert kwargs["memory_type"] == MEMORY_TYPE_METRIC


def test_generate_recommendation_never_writes_to_memory():
    # Deliberate: a strategic recommendation is not itself a Roadmap or
    # Metric write, and never a DecisionRecord (Product Decision
    # Specialist's exclusive write surface) - see module docstring.
    memory = _FakeMemory(package=_full_evidence_package())
    agent = _agent(memory_adapter=memory)
    request = StrategyRequest(operation=StrategyOperation.GENERATE_RECOMMENDATION, text="Should we invest further?", title="Investment")

    response = agent.process(request, _specialist_context())

    assert response.success is True
    assert memory.remembered == []


def test_generate_recommendation_without_evidence_is_honest_never_fabricated():
    memory = _FakeMemory()
    agent = _agent(memory_adapter=memory)
    request = StrategyRequest(operation=StrategyOperation.GENERATE_RECOMMENDATION, text="Should we build X?", title="X")

    response = agent.process(request, _specialist_context())

    assert response.success is True
    assert response.confidence < 0.5
    assert "discovery" in response.recommendations[0].lower() or "decision" in response.recommendations[0].lower()


def test_sequencing_roadmap_twice_appends_never_mutates():
    memory = _FakeMemory()
    agent = _agent(memory_adapter=memory)
    context = _specialist_context()

    agent.process(StrategyRequest(operation=StrategyOperation.SEQUENCE_ROADMAP, items=("Ship A",), horizon="now"), context)
    agent.process(StrategyRequest(operation=StrategyOperation.SEQUENCE_ROADMAP, items=("Ship A",), horizon="next"), context)

    assert len(memory.remembered) == 2
    assert "now" in memory.remembered[0][0]
    assert "next" in memory.remembered[1][0]
    assert memory.remembered[0] in memory.remembered  # first entry never removed or rewritten


# --- Portfolio Rule: never a new memory category, never cross-product reasoning ---------------


def test_assess_portfolio_never_introduces_a_new_memory_type():
    memory = _FakeMemory()
    agent = _agent(memory_adapter=memory)
    request = StrategyRequest(operation=StrategyOperation.ASSESS_PORTFOLIO, products=("Product A", "Product B"))

    agent.process(request, _specialist_context())

    memory_types = {kwargs["memory_type"] for _, kwargs in memory.remembered}
    assert memory_types == {MEMORY_TYPE_ROADMAP}
    assert "product_portfolio" not in memory_types


def test_assess_portfolio_uses_the_existing_remember_portfolio_write_path():
    memory = _FakeMemory()
    agent = _agent(memory_adapter=memory)
    request = StrategyRequest(operation=StrategyOperation.ASSESS_PORTFOLIO, products=("Product A",))

    agent.process(request, _specialist_context())

    content, kwargs = memory.remembered[0]
    assert kwargs["title"] == "Portfolio snapshot"


def test_assess_portfolio_requires_at_least_one_product():
    response = _agent().process(StrategyRequest(operation=StrategyOperation.ASSESS_PORTFOLIO), _specialist_context())
    assert response.success is False
    assert "product" in response.error.lower()


def test_assess_portfolio_never_performs_cross_product_comparison():
    # The structural proof Implementation_Plan.md's own Milestone 6
    # acceptance criteria requires: each product is queried and reported
    # on independently - no comparison, ranking, or conflict language
    # appears anywhere in the response.
    memory = _FakeMemory(
        per_query_packages={
            "Product A": _package_with_items([_memory_item(content="Roadmap item: Ship A. Horizon: now.", resource_id=10)]),
            "Product B": _package_with_items([_memory_item(content="Roadmap item: Ship B. Horizon: later.", resource_id=20)]),
        }
    )
    agent = _agent(memory_adapter=memory)
    request = StrategyRequest(operation=StrategyOperation.ASSESS_PORTFOLIO, products=("Product A", "Product B"))

    response = agent.process(request, _specialist_context())

    assert response.success is True
    # findings[0] is the scope_note (which legitimately *names* these
    # concepts to disclose they were NOT performed); the per-product
    # notes that follow are the actual reasoning content, and must never
    # reference another product or claim a cross-product judgment.
    per_product_notes = response.findings[1:]
    for forbidden in ("versus", " vs ", "trade-off across", "blocks", "depends on product"):
        assert not any(forbidden in note.lower() for note in per_product_notes)
    assert not any("product a" in note.lower() and "product b" in note.lower() for note in per_product_notes)
    # Two independent recall() calls, one per product - never one combined
    # cross-product query.
    queries = [call[0] for call in memory.retrieve_calls]
    assert "Product A" in queries
    assert "Product B" in queries


def test_assess_portfolio_response_always_discloses_scope():
    memory = _FakeMemory()
    agent = _agent(memory_adapter=memory)
    request = StrategyRequest(operation=StrategyOperation.ASSESS_PORTFOLIO, products=("Product A", "Product B"))

    response = agent.process(request, _specialist_context())

    assert "single-product inventory" in response.findings[0].lower()
    assert "v1 scope" in response.findings[0].lower() or "no cross-product" in response.findings[0].lower()


def test_never_writes_a_decision_record_or_delivery_artifact():
    from app.services.ai.agents.specialists.product_management.shared.types import (
        MEMORY_TYPE_DECISION,
        MEMORY_TYPE_DELIVERY_ARTIFACT,
    )

    memory = _FakeMemory(package=_full_evidence_package())
    agent = _agent(memory_adapter=memory)
    context = _specialist_context()

    agent.process(StrategyRequest(operation=StrategyOperation.SEQUENCE_ROADMAP, items=("Ship A",), horizon="now"), context)
    agent.process(StrategyRequest(operation=StrategyOperation.STRUCTURE_NORTH_STAR, title="WAU"), context)
    agent.process(StrategyRequest(operation=StrategyOperation.GENERATE_RECOMMENDATION, text="Invest more?", title="X"), context)
    agent.process(StrategyRequest(operation=StrategyOperation.ASSESS_PORTFOLIO, products=("Product A",)), context)

    memory_types = {kwargs["memory_type"] for _, kwargs in memory.remembered}
    assert MEMORY_TYPE_DECISION not in memory_types
    assert MEMORY_TYPE_DELIVERY_ARTIFACT not in memory_types


# --- Discovery/Decision/Delivery integration: shared memory only, never their own types --------


def test_module_never_imports_other_specialists_code():
    module = importlib.import_module(
        "app.services.ai.agents.specialists.product_management.strategy_portfolio.strategy_portfolio_agent"
    )
    tree = ast.parse(inspect.getsource(module))
    imported_modules = [node.module for node in ast.walk(tree) if isinstance(node, ast.ImportFrom) and node.module]
    assert not any("product_management.discovery" in name for name in imported_modules)
    assert not any("product_management.product_decision" in name for name in imported_modules)
    assert not any("product_management.delivery" in name for name in imported_modules)
    assert not any("specialists.research" in name for name in imported_modules)
    assert not any("personal_intelligence" in name for name in imported_modules)


def test_scoring_module_never_imports_product_decisions_scoring_module():
    module = importlib.import_module("app.services.ai.agents.specialists.product_management.strategy_portfolio.scoring")
    tree = ast.parse(inspect.getsource(module))
    imported_modules = [node.module for node in ast.walk(tree) if isinstance(node, ast.ImportFrom) and node.module]
    assert not any("product_decision" in name for name in imported_modules)


def test_consumes_discovery_and_decision_evidence_through_shared_memory_recall():
    memory = _FakeMemory(package=_full_evidence_package())
    agent = _agent(memory_adapter=memory)

    response = agent.process(
        StrategyRequest(operation=StrategyOperation.GENERATE_RECOMMENDATION, text="Users churn after onboarding", title="Onboarding"),
        _specialist_context(),
    )

    assert response.success is True
    assert set(response.sources) == {"1", "2"}


# --- Runtime / PromptBuilder integration -----------------------------------------------------


def test_runtime_request_carries_organization_id_and_parent_shared():
    runtime = _FakeRuntime()
    agent = _agent(runtime_adapter=RuntimeAdapter(runtime=runtime))
    context = _specialist_context()

    agent.process(StrategyRequest(operation=StrategyOperation.RECALL, text="roadmap"), context)

    assert len(runtime.requests) == 1
    assert runtime.requests[0].organization_id == 1
    assert runtime.requests[0].parent_shared is context.shared


def test_prompt_package_is_built_from_prompt_builder_and_contains_the_text():
    runtime = _FakeRuntime()
    agent = _agent(runtime_adapter=RuntimeAdapter(runtime=runtime))

    agent.process(
        StrategyRequest(operation=StrategyOperation.ASSESS_TRADEOFFS, text="speed vs polish"), _specialist_context()
    )

    prompt_package = runtime.requests[0].prompt_package
    rendered = " ".join(section.content for section in prompt_package.sections)
    assert "speed vs polish" in rendered


def test_summary_uses_runtime_generated_text_when_available():
    conversation_response = ConversationResponse(
        text="A clear strategic framing.",
        response=ProviderResponse(metadata=AIResponseMetadata(provider=ProviderName.OPENAI, model="fake")),
    )
    runtime = _FakeRuntime(response=RuntimeResponse(success=True, conversation_response=conversation_response))
    agent = _agent(runtime_adapter=RuntimeAdapter(runtime=runtime))

    response = agent.process(StrategyRequest(operation=StrategyOperation.RECALL, text="roadmap"), _specialist_context())

    assert response.summary == "A clear strategic framing."


# --- event ordering --------------------------------------------------------------------------


def test_events_are_emitted_in_order_for_sequence_roadmap():
    events = []
    publisher = StrategyEventPublisher()
    publisher.subscribe(events.append)
    agent = _agent(event_publisher=publisher)

    agent.process(
        StrategyRequest(operation=StrategyOperation.SEQUENCE_ROADMAP, items=("Ship A",), horizon="now"), _specialist_context()
    )

    event_types = [event.event_type for event in events]
    assert event_types == [
        StrategyEventType.REQUEST_STARTED,
        StrategyEventType.PRECEDENT_RETRIEVED,
        StrategyEventType.ROADMAP_REVISED,
        StrategyEventType.REQUEST_COMPLETED,
    ]


def test_events_are_emitted_in_order_for_assess_portfolio():
    events = []
    publisher = StrategyEventPublisher()
    publisher.subscribe(events.append)
    agent = _agent(event_publisher=publisher)

    agent.process(
        StrategyRequest(operation=StrategyOperation.ASSESS_PORTFOLIO, products=("Product A",)), _specialist_context()
    )

    event_types = [event.event_type for event in events]
    assert event_types == [
        StrategyEventType.REQUEST_STARTED,
        StrategyEventType.PRECEDENT_RETRIEVED,
        StrategyEventType.PORTFOLIO_ASSESSED,
        StrategyEventType.REQUEST_COMPLETED,
    ]
    assert StrategyEventType.CROSS_PRODUCT_CONFLICT_SURFACED not in event_types


def test_every_event_carries_execution_id_correlation_id_and_agent_id():
    events = []
    publisher = StrategyEventPublisher()
    publisher.subscribe(events.append)
    agent = _agent(event_publisher=publisher)
    context = _specialist_context()

    agent.process(StrategyRequest(operation=StrategyOperation.RECALL, text="x"), context)

    assert all(event.execution_id == context.execution_id for event in events)
    assert all(event.correlation_id == context.correlation_id for event in events)
    assert all(event.agent_id == "strategy_portfolio" for event in events)


def test_failed_run_emits_request_failed_instead_of_completed():
    events = []
    publisher = StrategyEventPublisher()
    publisher.subscribe(events.append)
    agent = _agent(event_publisher=publisher, runtime_adapter=RuntimeAdapter(runtime=_FakeRuntime(raises=ValueError("boom"))))

    agent.process(StrategyRequest(operation=StrategyOperation.RECALL, text="x"), _specialist_context())

    event_types = [event.event_type for event in events]
    assert StrategyEventType.REQUEST_FAILED in event_types
    assert StrategyEventType.REQUEST_COMPLETED not in event_types


# --- state transitions ------------------------------------------------------------------------


def test_strategy_state_ends_at_idle_after_a_successful_run():
    agent = _agent()
    agent.process(StrategyRequest(operation=StrategyOperation.RECALL, text="x"), _specialist_context())
    assert agent.strategy_state.state == StrategyState.IDLE


def test_strategy_state_ends_at_idle_after_a_failed_run():
    agent = _agent(runtime_adapter=RuntimeAdapter(runtime=_FakeRuntime(raises=ValueError("boom"))))
    agent.process(StrategyRequest(operation=StrategyOperation.RECALL, text="x"), _specialist_context())
    assert agent.strategy_state.state == StrategyState.IDLE


def test_strategy_state_ends_at_idle_after_an_early_validation_error():
    agent = _agent()
    response = agent.process(StrategyRequest(operation=StrategyOperation.ASSESS_PORTFOLIO), _specialist_context())
    assert response.success is False
    assert agent.strategy_state.state == StrategyState.IDLE


def test_agent_can_process_multiple_requests_in_sequence():
    agent = _agent()
    context = _specialist_context()
    for _ in range(3):
        response = agent.process(StrategyRequest(operation=StrategyOperation.RECALL, text="x"), context)
        assert response.success is True
    assert agent.strategy_state.state == StrategyState.IDLE


# --- failure handling -------------------------------------------------------------------------


def test_a_raising_memory_adapter_produces_a_failed_response_not_a_raised_exception():
    memory = _FakeMemory()
    memory.raises_on_remember = ValueError("db unavailable")
    agent = _agent(memory_adapter=memory)
    response = agent.process(
        StrategyRequest(operation=StrategyOperation.SEQUENCE_ROADMAP, items=("Ship A",), horizon="now"), _specialist_context()
    )
    assert response.success is False
    assert "db unavailable" in response.error


def test_a_failing_runtime_call_produces_a_failed_response_with_zero_confidence():
    agent = _agent(
        runtime_adapter=RuntimeAdapter(runtime=_FakeRuntime(response=RuntimeResponse(success=False, error="provider down")))
    )
    response = agent.process(StrategyRequest(operation=StrategyOperation.RECALL, text="x"), _specialist_context())
    assert response.success is False
    assert response.confidence == 0.0
    assert response.error == "provider down"


def test_maximum_depth_exceeded_fails_immediately_without_touching_any_collaborator():
    memory = _FakeMemory()
    runtime = _FakeRuntime()
    agent = _agent(memory_adapter=memory, runtime_adapter=RuntimeAdapter(runtime=runtime))
    context = _specialist_context(agent_context=_agent_context(delegation_depth=agent.policy.maximum_depth))

    response = agent.process(StrategyRequest(operation=StrategyOperation.RECALL, text="x"), context)

    assert response.success is False
    assert "depth" in response.error.lower()
    assert memory.retrieve_calls == []
    assert runtime.requests == []


def test_compare_opportunities_requires_at_least_two():
    response = _agent().process(
        StrategyRequest(operation=StrategyOperation.COMPARE_OPPORTUNITIES, items=("only-one",)), _specialist_context()
    )
    assert response.success is False
    assert "at least two" in response.error.lower()


def test_unsupported_raw_operation_value_raises_before_construction():
    with pytest.raises(ValueError):
        StrategyOperation("not_a_real_operation")
