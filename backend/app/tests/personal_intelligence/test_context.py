"""build_personal_intelligence_context() - a thin SpecialistContext
composer, structurally identical to build_research_context(). No new
context type, no execution-identity duplication.
"""
...
def test_carries_the_given_agent_context_through_unchanged():
    ...
    context = build_personal_intelligence_context(agent_context, None)
    assert context.agent_context is agent_context


def test_carries_the_given_request_through_unchanged():
    request = SpecialistRequest(objective="Remember my timezone")
    context = build_personal_intelligence_context(AgentContext(organization_id=1), request)
